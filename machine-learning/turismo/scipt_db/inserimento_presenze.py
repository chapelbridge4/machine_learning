...
select_query_agriturismi = """
    SELECT Regione, Anno, Presenze
    FROM Presenze_negli_agriturismi_in_Italia_per_regione
"""

select_query_alberghiero = """
    SELECT Regione, Anno, Presenze
    FROM Presenze_negli_alberghi_in_Italia_per_regione
"""

select_query_campeggi = """
    SELECT Regione, Anno, Presenze
    FROM Presenze_nei_campeggi_in_Italia_per_regione
"""
...
for select_query, settore in [(select_query_agriturismi, 'agriturismo'), (select_query_alberghiero, 'alberghiero'), (select_query_campeggi, 'campeggio')]:
    ...
    insert_query = """
        INSERT INTO presenze (dati, id_regione, id_anno, settore)
        VALUES (?, ?, ?, ?)
    """

    for row in rows:
        regione, anno, presenze = row
        region_id = region_name_to_id.get(regione)
        year_id = year_to_id.get(anno)

        if region_id is not None and year_id is not None:
            cursor.execute(insert_query, (presenze, region_id, year_id, settore))

    # Committo le modifiche per questa tabella di origine
    conn.commit()
